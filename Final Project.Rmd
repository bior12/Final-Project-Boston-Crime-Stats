...
```{r table creation, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'hide'}
...
average_lat_lng <- function(t) {
  ...
  return(newlist) #returns both the mean latitude and longitude found in the given datatable
}
paste_faster <- function(t) {
  return <- paste("(",round(t$alat,5),",",round(t$alng,5),")",sep="") #returns the nice looking lat / lng in a pair of numbers
}
difference <- function(t1,t2){
  return <- paste("(",round(t2$alat-t1$alat,8),",",round(t2$alng-t1$alng,8),")",sep="") # returns the differences between two datasets
}

```

```{r info for multiple years}
avg_2015<-average_lat_lng(data_for_2015) #a series of function calls
...
```{r heatmap calls}
display_heatmap(data_for_2015) #function call to make the heat maps for different datasets
