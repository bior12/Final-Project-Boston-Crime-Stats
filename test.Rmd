...
#"ggspatial", "libwgeom", "sf", "rnaturalearth", "rnaturalearthdata", "rworldxtra"))
```
```{r}
...
```
```{r}
latt<-location_of_incident%>%select(Lat)%>%filter(Lat>40)
lngg<-location_of_incident%>%select(Long)%>%filter(Long<(-70))
latt
lngg
```
```{r}
lat_avg=mean(latt$Lat,na.rm = T)
lng_avg=mean(lngg$Long,na.rm = T)
lat_avg
lng_avg
```

```{r}
ggplot(data=location_of_incident%>%head(100000),aes(x=Long,y=Lat))+geom_density_2d()
```


```{r}
locationMap <-
  leaflet(location_of_incident%>%head(10)) %>%   # like ggplot()
