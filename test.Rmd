...
```{r}
#yee<-boston_crime_statistics%>%
#  mutate("new_date"=ymd(as.Date(date)))
...
first_date<-boston_crime_statistics%>%
  dplyr::select(date,YEAR,MONTH,DAY_OF_WEEK,HOUR)%>%
  arrange(date)%>%
  head(1)
...
last_date[1, "date"]-first_date[1, "date"]
```
```{r}
...
for (n in 2015:2018){
  cat("Data for Year",n,"\n")
  lang %>% 
    filter(lang$date > as.POSIXct(as.Date(as.character(cat(n,"-01-01 01:00:00")))))%>%
    filter(lang$date < as.POSIXct(as.Date(as.character(cat(n,"-12-31 23:59:00")))))
}

ggplot(data=lang,aes(x=lng,y=lat))+geom_density_2d()
```
